...
async function getAccessoryDump(instance) {
  ...
  if ((filter && filter === instance.host + ':' + instance.port) || !filter) {
    try {
      ...
      switch (response.status) {
        ...
        case 470:
          ...
          throw new Error('homebridge is not running in insecure mode with -I', response.status);
          break;
        default:
    ...
    debugGetAcc('Filtered HAP instance address: %s -> %s', instance.txt.md, instance.url);
  }
}
